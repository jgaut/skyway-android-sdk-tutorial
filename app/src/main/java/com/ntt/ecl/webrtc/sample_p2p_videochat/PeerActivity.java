...
public class PeerActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		PeerActivity.API_KEY=MyAppProperties.getProperty("Skyway.api_key");
		PeerActivity.DOMAIN=MyAppProperties.getProperty("Skyway.domain");

		//Volume media
		audioManager =
				(AudioManager)getSystemService(this.getApplicationContext().AUDIO_SERVICE);
        ...
        MyDataActivity.setPeerActivity(this);
		...
		_peer.on(Peer.PeerEventEnum.OPEN, new OnCallback() {
			...
			public void onCallback(Object object) {
				...
				Log.i(TAG, "I am " +_strOwnId);
				MyDataActivity.setSkyWayId(_strOwnId);
                ...
                if(extras !=null)
                {
                    if(extras.getString("skyWayId").equals("0")){
                        String message = "call_me:"+MyDataActivity.getSkyWayId();
                        Log.i(TAG, message);
                        MyDataActivity.getMyWebSocketClient().send(message);

                    }else{
                        onPeerSelected(extras.getString("skyWayId"));
                    }

                }
				//showPeerIDs();
				//Log.i(TAG, "showPeerIDs !!");
		...
		_peer.on(Peer.PeerEventEnum.CALL, new OnCallback() {
			...
			public void onCallback(Object object) {
				...
				option.audioBandwidth=64;
				option.audioCodec="PCMA";
				_mediaConnection.answer(_localStream, option);
		...
		btnAction.setOnClickListener(new View.OnClickListener()	{
			...
			public void onClick(View v)	{
				...
				if (!_bConnected) {
                    ...
                    _peer.disconnect();
                    _peer.destroy();
                    MyDataActivity.getPeerActivity().finish();
				}
		...
		Button muteAction = (Button)findViewById(R.id.muteAction);
		muteAction.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v)	{
		...
		Button speakerPhoneAction = (Button)findViewById(R.id.speakerPhoneAction);
		muteAction.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v)	{
	...
	void startLocalStream() {
		...
		Log.i(TAG, "nb video tracks : "+_localStream.getVideoTracks());
	}
